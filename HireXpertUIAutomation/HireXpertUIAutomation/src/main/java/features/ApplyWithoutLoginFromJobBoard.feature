...
And Select all mandatory filters and click on search button "<Industry>" "<JobDesignation>" "<JobRole>" "<experience>" "<JobCity>"
And Verify the details on Job card displayed on left side pannel "<JobTitle>" "<OrganizationName>" "<JobLocation>" "<JobCity>" "<JobCountry>" "<MinExp>" "<MaxExp>" "<JobBudget>" "<Industry>" "<JobDesignation>"
And Click on Job Details button of a respective job where you want to apply
#And Verify all the details of the job after clicking on Job Details button "<JobTitle>" "<JobDesignation>" "<JobLocation>" "<JobCity>>" "<JobCountry>" "<MinExp>" "<MaxExp>" "<JobBudget>" "<Industry>" "<JobRole>"
